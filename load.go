...
func processEntries(harfile string, worker int, entries chan Entry, results chan TestResult, ignoreHarCookies bool, insecureSkipVerify bool, stop chan bool) {
	...
	for {
		...
		case entry := <-entries:
			...
			resultList = append(resultList, trr)
				
			results <- tr
		}
		iter++

		outResultList = append(outResultList, resultList)

		jsonData, _ := json.Marshal(resultList)
		fmt.Println("Ha Ha Start")
		fmt.Println(string(jsonData))
		fmt.Println("Ha Ha End")
		currentTime := time.Now()
		err := ioutil.WriteFile("./test/"+currentTime.Format("2006-01-02 3:4:5")+".json", jsonData, 0644)
		if (err != nil){
			fmt.Println("failed")
		}
